...
export class PlaybarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    const progressSubscription = this.progressService.playedLengthMillis$.subscribe(
      (value) => {
        if (this.changing) {
          return;
        }
    ...
    const trackDurationSubscription =
      this.progressService.currentDurationMillis$.subscribe((value) => {
